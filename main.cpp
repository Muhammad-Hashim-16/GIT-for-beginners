...
void update_hash_vector(string blobHash) {
    indexHash.push_back(blobHash);
}
...
// -------------- Running loop for all the files --------------
    ...
    // Check if the hash of the file is already in index vector or not

        for (int i=0; i<indexHash.size(); i++) {
            if (stringHash == indexHash[i]) continue;
        }
